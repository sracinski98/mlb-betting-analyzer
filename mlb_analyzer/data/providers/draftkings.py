...
class DraftKingsProvider(OddsProvider):
    ...
    DRAFTKINGS_MLB_URL = f"{BASE_URL}/sdm/mlb/getschedule"
    ...
    def _get_events(self) -> Dict:
        ...
        try:
            self.logger.info(f"Fetching events from DraftKings: {self.DRAFTKINGS_MLB_URL}")
            response = self.session.get(self.DRAFTKINGS_MLB_URL, timeout=10)
            self.logger.info(f"DraftKings response status: {response.status_code}")
            
            # Log the response status and content type
            self.logger.info(f"DraftKings response status: {response.status_code}")
            self.logger.info(f"DraftKings response headers: {response.headers}")
            
            if response.status_code != 200:
                self.logger.error(f"Error response from DraftKings: {response.text}")
                return {}
            ...
            data = response.json()
            self.logger.info(f"Received data with keys: {list(data.keys()) if isinstance(data, dict) else 'Not a dict'}")
            return data
            
        except requests.exceptions.RequestException as e:
            self.logger.error(f"Network error fetching DraftKings events: {str(e)}")
            return {}
        except ValueError as e:
            self.logger.error(f"Invalid JSON response from DraftKings: {str(e)}")
            return {}
        except Exception as e:
            self.logger.error(f"Unexpected error fetching DraftKings events: {str(e)}")
